...
public class LPlusGServiceThread extends Thread {
    private static final String TAG = LPlusGServiceThread.class.getSimpleName();
    ...
    public LPlusGServiceThread(Socket s, EnergyMeter device) {
        this.device = device;
        myClientSocket = s;
        device.setMakeString(Constants.LPG_STR);
...
//
//                if (responseBytes[0] == Commands.lnt_resp_akg1[0]
//                        || responseBytes[0] == Commands.lnt_resp_akg2[0]) {
...
//            }
            ...
            if (error.get()) {
                Log.e(TAG, "Error while reading LNT data");
                dout.close();
                inputStream.close();
                myClientSocket.close();
                device.setReadStatus(Constants.FAILED);
            ...
            Log.e(TAG, "IOExcepiton while running LNT thread.");
            e.printStackTrace();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } finally {
