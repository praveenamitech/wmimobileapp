...
public class LNTLegacyServiceThread extends Thread {
    private static final String TAG = LNTLegacyServiceThread.class.getSimpleName();
    ...
    public LNTLegacyServiceThread(Socket s, EnergyMeter device) {
        this.device = device;
        myClientSocket = s;
        device.setMakeString(Constants.LNT_lEGACY_STR);
    ...
    public void run() {
        ...
        new Thread(() -> {
            for (int i = 0; i < 20; i++) {
                ...
                try {
                    write(Commands.lntlegacy_cmd_first);
                    Thread.sleep(1000);
        ...
        new Thread(() -> {
            for(int i=0;i<2;i++) {
                ...
                try {
                    ...
                    if (responseBytes[0] == Commands.lntlegacy_resp_akg1[0] || responseBytes[0] == Commands.lntlegacy_resp_akg2[0]) {

                        gotInitData.set(true);
                        Log.d(TAG, "got initial response: " + CustomUtils.bytesToHexString(responseBytes));
                        commandResponses.add(responseBytes);
                        //akgBuffer=new byte[0];
        ...
        try {
            ...
            if (error.get()) {
                Log.e(TAG, "Error occurred after first command1");
                dout.close();
                inputStream.close();
            ...
            write(Commands.lntlegacy_cmd_second);
            ...
            if (error.get()) {
                Log.e(TAG, "Error occurred after first command2");
                dout.close();
                inputStream.close();
                myClientSocket.close();
                device.setReadStatus(Constants.FAILED);
            ...
            write(Commands.lntlegacy_cmd_third);
            sleep(Constants.MIN_DELAY);
            ...
            if (error.get()) {
                Log.e(TAG, "Error while reading lntlegacy data");
                dout.close();
                inputStream.close();
                myClientSocket.close();
                device.setReadStatus(Constants.FAILED);
            ...
            Log.e(TAG, "IOExcepiton while running lntlegacy thread.");
            e.printStackTrace();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } finally {
